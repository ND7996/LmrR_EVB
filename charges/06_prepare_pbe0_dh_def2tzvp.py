...
METHOD_DIR = OUT_DIR / "PBE0-DH_def2TZVP"
...
def main():
    for label in STRUCTURE_SETS:
        for state in ("RS","TS","PS"):
            ...
            input_file = state_dir / "input" / f"{label}_{state}_PBE0-DH_def2TZVP_SP.inp"
            write_input(input_file, "PBE0-DH def2-TZVP SP TightSCF", geometry)
            (state_dir/"README.txt").write_text(
                "PBE0-DH double-hybrid functional / def2-TZVP. Test one representative job before submitting all jobs.\n"
                "Single-point benchmark on the optimized reference geometry.\n",
                encoding="utf-8")
    print(f"Prepared PBE0-DH_def2TZVP jobs in {METHOD_DIR}")
