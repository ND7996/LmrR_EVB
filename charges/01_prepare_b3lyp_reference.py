...
METHOD_DIR = OUT_DIR / "B3LYP_def2SVP"

def main():
    for label, pdbs in STRUCTURE_SETS.items():
        for state, pdb_name in zip(("RS","TS","PS"), pdbs):
            pdb = CHARGES_DIR / pdb_name
            if not pdb.exists():
                print(f"WARNING missing: {pdb}")
                continue
            state_dir = METHOD_DIR / label / state
            xyz = state_dir / "geometry" / f"{label}_{state}_initial.xyz"
            mp = state_dir / "geometry" / f"{label}_{state}_pdb_to_xyz_map.csv"
            write_xyz(pdb, xyz, mp)
            if state == "TS":
                route = "B3LYP D3BJ def2-SVP OptTS NumFreq TightSCF"
            else:
                route = "B3LYP D3BJ def2-SVP Opt NumFreq TightSCF"
            write_input(state_dir / "input" / f"{label}_{state}_B3LYP_def2SVP.inp", route, xyz)
            (state_dir/"README.txt").write_text(
                "B3LYP/def2-SVP reference calculation. "
                "RS/PS optimized with Opt+NumFreq; TS with OptTS+NumFreq.\n",
                encoding="utf-8")
    print(f"Prepared B3LYP/def2-SVP reference jobs in {METHOD_DIR}")
