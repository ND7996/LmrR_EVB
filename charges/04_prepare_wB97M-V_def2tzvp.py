...
METHOD_DIR = OUT_DIR / "wB97M-V_def2TZVP"
...
def main():
    for label in STRUCTURE_SETS:
        for state in ("RS","TS","PS"):
            ...
            input_file = state_dir / "input" / f"{label}_{state}_wB97M-V_def2TZVP_SP.inp"
            write_input(input_file, "wB97M-V def2-TZVP SP TightSCF", geometry)
            (state_dir/"README.txt").write_text(
                "omegaB97M-V range-separated hybrid meta-GGA with VV10 nonlocal correlation / def2-TZVP. No D3BJ is added.\n"
                "Single-point benchmark on the optimized reference geometry.\n",
                encoding="utf-8")
    print(f"Prepared wB97M-V_def2TZVP jobs in {METHOD_DIR}")
